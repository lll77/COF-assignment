/**
 * 
 */
package com.capitalone.codingtask.util;

/**
 * 
 * Class holding TransactionBot constants.
 * 
 * @author MalgariV
 *
 */
public class TransactionBotConstants {
	
	public static final String ALL_TRANSACTIONS_ENDPOINT = "allTransactions.endpoint";
	
	public static final String ALL_TRANSACTIONS_ENDPOINT_VALIDATION_MSG = "Endpoint should not be null.";
	
	public static final String AUTH_TOKEN = "authToken";
	
	public static final String AUTH_TOKEN_VALIDATION_MSG = "Authentication token should not be null.";
	
	public static final String USER_ID = "userId";
	
	public static final String USER_ID_VALIDATION_MSG = "User id should not be null.";
	
	public static final String API_TOKEN_VALUE = "apiToken";
	
	public static final String API_TOKEN_VALIDATION_MSG = "API Token should not be null.";
	
	public static final String REQUEST_PARAM_ENDPOINT = "endPoint";
	
	public static final String REQUEST_PARAM_AUTHTOKEN = "authToken";
	
	public static final String REQUEST_PARAM_USERID = "userId";
	
	public static final String REQUEST_PARAM_APITOKEN = "apiToken";
	
	public static final String REQUEST_PARAM_EXCLUDE_DONUTS = "excludeDonuts";
	
	public static final String DUNKIN_STORE = "DUNKIN #336784";
	
	public static final String KRISPY_STORE = "Krispy Kreme Donuts";
	
}